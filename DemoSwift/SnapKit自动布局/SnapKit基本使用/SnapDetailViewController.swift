    ...
    func showSnap()  {
        ...
        case 101:
            ...
            box3.snp.makeConstraints({ (make) in
                make.top.equalTo(box1)
                make.width.height.equalTo(50)
                make.left.equalTo(box1.snp.right).offset(20)
            })
        ...
        case 102:
            ...
            box3.snp.makeConstraints({ (mark) in
                mark.size.equalTo(CGSize(width: 50, height: 50))
                mark.right.equalTo(box1.snp.right).offset(-20)
                mark.bottom.equalTo(-20)
            })
        ...
        case 105:
            ...
            box2.snp.makeConstraints({ (make) in
                make.size.equalTo(box1).multipliedBy(0.8)   //大小为box1一半
                make.centerX.equalTo(box1)
                make.top.equalTo(box1.snp.bottom).offset(20)     
            })
            
            box3.snp.makeConstraints({ (make) in
                make.size.equalTo(box2).multipliedBy(0.8)
                make.centerX.equalTo(box2)
                make.top.equalTo(box2.snp.bottom).offset(30)
            })
